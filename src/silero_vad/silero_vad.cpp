...
namespace xiaozhi {
    ...
    VoiceDetector::VoiceDetector()
        ...
        session = std::make_shared<Ort::Session>(env, setting->config["VAD"]["SileroVAD"]["model_path"].as<std::string>().c_str(), session_options);
    
        // 初始化隐藏状态
        state.resize(2 * 128, 0.0f);
    }
    ...
    float VoiceDetector::predict(std::vector<float> &pcm_data) {
        // 准备 Silero-VAD 输入张量
        std::vector<int64_t> input_shape = {1, window_size};
        std::vector<int64_t> sr_shape = {1};
        std::vector<int64_t> state_shape = {2, 1, 128};
    
        auto input_tensor = Ort::Value::CreateTensor<float>(memory_info, pcm_data.data(), window_size, 
                                                            input_shape.data(), input_shape.size());
        auto sr_tensor = Ort::Value::CreateTensor<int64_t>(memory_info, &sample_rate, 1, 
                                                            sr_shape.data(), sr_shape.size());
        auto state_tensor = Ort::Value::CreateTensor<float>(memory_info, state.data(), state.size(), 
        ...
        auto output_tensors = session->Run(Ort::RunOptions{nullptr}, input_names, input_tensors.data(), 3, 
                                            output_names, 2);
